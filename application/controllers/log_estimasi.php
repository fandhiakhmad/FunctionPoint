...
class Log_estimasi extends CI_Controller
{
    ...
    public function daftar_log_estimasi()
    {
        ...
        $data['content'] = $this->load->view('frontend_fp/daftar_log_estimasi', $isi, true);
    ...
    public function form_effort2()
    {
        ...
        $role['role']      = $this->session->userdata('role');
		$data['menu_kiri'] = $this->load->view('backend_fp/menu_kiri',$role, true);
        $data['content']    = $this->load->view('frontend_fp/form_effort', $isi, true);
   ...
   public function form_effort($id_aplikasi)
    {
        ...
        $role['role']      = $this->session->userdata('role');
		$data['menu_kiri'] = $this->load->view('backend_fp/menu_kiri',$role, true);
        $data['content']    = $this->load->view('frontend_fp/form_effort2', $isi, true);
    ...
    public function add_effort_real2()
    {
		...
		while($index<=$jumlah_aktivitas) {
			 ...
			 if(is_numeric($tim)==false){
				 ...
				 $isi_kosongtim=true;
				 $errtim [$index]=$index;
				  $status["pesantim".$index.""]      	= "Harus bilangan positif";
			 }
			 $waktu = $this->input->post('waktu'.$index);
			 if(is_numeric($waktu)==false){
				 $isi_kosongwaktu=true;
				 ...
				 $status["pesanwaktu".$index.""]      	= "Belum diisi";
				  ...
				  $isi_kosongwaktu=true;
				 $errwaktu [$index]=$index;
				 $status["pesanwaktu".$index.""]      	= "Harus bilangan positif";
			 }
			 ...
			 $hari = $this->input->post('hari'.$index);
			  if(is_numeric($hari)==false){
				 ...
				 $isi_kosonghari=true;
				  $errhari [$index]=$index;
				 $status["pesanhari".$index.""]      	= "Harus bilangan positif";
			 }
    ...
    public function print_penawaran($id_aplikasi)
    {
        ...
        $this->load->view('print/dok_penawaran_pdf', $isi, true);
    }
	...
	public function request_validasi($id_aplikasi)
	{
		...
		$isi['role']      = $this->session->userdata('role');
        $data['content'] = $this->load->view('frontend_fp/request_validasi', $isi, true);
        $this->load->view('/backend_fp/main', $data);
	}
}
